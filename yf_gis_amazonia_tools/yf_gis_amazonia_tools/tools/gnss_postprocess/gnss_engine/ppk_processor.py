...
class PPKProcessor(QThread):
    ...
    def _execute(self, cmd: list) -> bool:
        ...
        En Windows, rutas con espacios requieren entrecomillado explícito.
        Usa subprocess.run (más robusto que Popen en QThread).
        """
        try:
            # En Windows, construir cadena con comillas para rutas con espacios
            if platform.system() == 'Windows':
                cmd_str = ' '.join(
                    f'"{c}"' if (' ' in c or '(' in c) else c
                    for c in cmd
                )
                self.log.emit(f'  [CMD] {cmd_str}', 'info')
